...
namespace p
{
	...
	{
		// the frame type
		public static  byte FRAME_ACK = 0x06;
		public static  byte FRAME_NACK = 0x15;
		public static  byte FRAME_ENQ = 0x05;



		public static  byte FRAME_EOT = 0x04;
		public static  byte FRAME_ETX = 0x03;

		public static  byte FRAME_PHONE = 0x50;
		public static  byte FRAME_RSP = 0x52;
		public static  byte FRAME_TOF_PRINT = 0x44;
		public static  byte FRAME_TOF_PWD = 0x4D;
		public static  byte FRAME_TOF_PROCESSDATA = 0x50;
		public static  byte FRAME_MSR= 0x48;


		public static  int TAG_OUTPUT_PHONE = 161; //a1
		public static  int TAG_LABEL_PHONE = 162;//a2
		public static  int TAG_ITEMS_PHONE = 163;//a3

		public static  int TAG_INPUT_B1 = 177;//b1;
		public static  int TAG_INPUT_B2 = 178;//b2
		public static  int TAG_INPUT_B3 = 179;//b3
		public static  int TAG_INPUT_B4 = 180; //b4

		public static  int BYTES_OF_DATA_LENGTH = 4;

		public static  byte NO_CRYPTO = (byte) 0x00;

		// the command type
		public  static int FRAME_REQUEST = 1;
		public  static int FRAME_RESPONSE = 2;

		public  static byte FONT_SMALL = 1;//������
		public  static byte FONT_BIG = (byte)0xC0; //0x08 = Double-height mode  selected 0x10 = Double-width mode selected
		public  static byte FONT_DEFAULT = 0;
...
*/
	...
	//		Log.i("len",len+"");
			...
			if (a != null)
				System.Array.Copy(a, 0, result, 0, aL);
			if (b != null)
				System.Array.Copy(b, 0, result, aL, bL);
			if(c != null)
				System.Array.Copy(c, 0, result, aL+bL, cL);
			if(d != null)
				System.Array.Copy(d, 0, result, aL+bL+cL, dL);
   ...
   */
		private static string bytes2charsUTF8(byte[] buf, int offset, int length, bool bigEndian)
		{
			int len=0;
		...
		private static byte[] char2ByteUTF8(String input, int offset, int length)
		{
			...
			System.Array.Copy(output, 0, ret, 0, outLength);
			return ret;    
		}
		...
		public static byte[] buildMSRData(byte[] track1, byte[] track2, byte[] track3) 
		{
			...
			if (length > 0) {
				...
				if (track1 != null && track1.Length > 0) {
					...
					length += 4;
					System.Array.Copy(track1, 0, buffer, length, track1.Length);
				...
				if (track2 != null && track2.Length > 0) {
					...
					length += 4;
					System.Array.Copy(track2, 0, buffer, length, track2.Length);
				...
				if (track3 != null && track3.Length > 0) {
					...
					length += 4;
					System.Array.Copy(track3, 0, buffer, length, track3.Length);
		...
		public static byte[] convertPrintData(String str, int offset, int length, byte languageSet, byte fontSet,byte align,byte linespace)
		{
			...
			//	System.Array.Copy(str.GetBytes(), offset, buffer, 0, length);
			}
 ...
 */
		...
		public static int TLVAppend(short tag, byte[] src, int src_offset, byte[] dst, int dst_offset, int length)
		{
			...
			if ((buffer[0] & 0x1F) == 0x1F) {
				System.Array.Copy(buffer, 0, dst, i, 2);
				i ++;
			} else {
				dst[i] = buffer[1];
			}
			...
			i ++;
			System.Array.Copy(src, src_offset, dst, i, length);
			i = i + length;
